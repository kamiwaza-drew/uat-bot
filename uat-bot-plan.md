# UAT Bot Plan: Containerized Vision-Guided Stress Testing
...
- `docker-compose.yml` for standalone usage:
  ...
  services:
    uat-bot:
      ...
      volumes:
        - ./data:/data
        - ./scenarios:/app/uat_bot/scenarios/custom
  ```
...
- On run start, creates N test users via `POST /api/v1/users/local`:
  ...
  async def provision_test_users(count: int, role_distribution: dict) -> list[TestUser]:
      ...
      """
  ```
- Usernames: `uat-bot-{run_id[:8]}-{role}-{n}` (e.g., `uat-bot-a1b2c3d4-editor-2`)
- On run end, deletes all created users via `DELETE /api/v1/users/{user_id}`
- Cleanup on container start: find and delete any orphaned `uat-bot-*` users from crashed runs
...
| `UAT_PROFILE` | No | `smoke` | Default stress profile |
| `UAT_AUTO_RUN` | No | `false` | Start run on container boot (for CI) |
| `UAT_DATA_DIR` | No | `/data` | Results storage path |
| `UAT_PORT` | No | `18090` | Control plane listen port |
| `UAT_VISION_MODEL` | No | `claude-sonnet-4-6` | Vision model for validation |
| `UAT_VISION_MODEL_COMPLEX` | No | `claude-opus-4-6` | Vision model for exploration |
| `UAT_MAX_WORKERS` | No | `20` | Max concurrent browser contexts |
| `UAT_SCREENSHOT_QUALITY` | No | `80` | JPEG quality (0-100, or `png`) |
