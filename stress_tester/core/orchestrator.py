...
from stress_tester.core.run_state import RunState
from stress_tester.core.user_manager import KamiwazaUserManager
from stress_tester.core.worker import Worker
from stress_tester.models import ReviewRunRequest, RunCreateRequest, RunDetail, RunStatus, RunSummary
from stress_tester.review import ReviewPlanner, build_review_summary
from stress_tester.reporting.analyzer import RunAnalyzer
...
from stress_tester.vision.client import VisionClient
...
class StressOrchestrator:
    ...
    async def _create_state(
        ...
        *,
        ...
        run_dir = self.settings.stress_tester_data_dir / "runs" / run_id
        ...
        task = asyncio.create_task(self._execute_run(state), name=f"stress-run-{run_id}")
        state.set_task(task)
        return state
    ...
    async def _execute_run(self, state: RunState) -> None:
        ...
        screenshot_manager = ScreenshotManager(
            ...
            quality_setting=self.settings.stress_tester_screenshot_quality,
        )
        ...
        try:
            ...
            if state.config.vision_enabled and self.settings.anthropic_api_key:
                vision_client = VisionClient(
                    ...
                    model=self.settings.stress_tester_vision_model,
                    model_complex=self.settings.stress_tester_vision_model_complex,
                )

            semaphore = asyncio.Semaphore(self.settings.stress_tester_max_workers)
