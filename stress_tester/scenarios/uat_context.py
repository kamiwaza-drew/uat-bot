...
from stress_tester.models import UATGuidanceBundle, UATGuidanceDoc
...
class UATContextLoader:
    ...
    def _repo_roots(self) -> list[Path]:
        roots: list[Path] = []
        patterns = [x.strip() for x in self.settings.stress_tester_extension_roots.split(",") if x.strip()]
    ...
    def load_bundle(self, component: str | None = None) -> UATGuidanceBundle:
        ...
        for uat_dir in self.discover_uat_dirs(component=component):
            ...
            for file_path in files:
                if len(docs) >= self.settings.stress_tester_guidance_max_files:
                    break
                ...
                docs.append(
                    UATGuidanceDoc(
                        path=str(file_path),
                        content=trimmed[: self.settings.stress_tester_guidance_max_chars_per_file],
                    )
                )
